...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in SUBSCRIBERS:
        ...
        msg = "❌ Status: NOT SUBSCRIBED\n\nSila tekan Subscribe atau Alert ON untuk aktifkan."
    await update.message.reply_text(msg, reply_markup=main_menu())
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "signal_btc":
        ...
        await query.message.reply_text("📈 Carta BTC:\nhttps://www.tradingview.com/chart/?symbol=BTCUSD", reply_markup=main_menu())

    elif data == "carta_gold":
        await query.message.reply_text("📈 Carta GOLD:\nhttps://www.tradingview.com/chart/?symbol=XAUUSD", reply_markup=main_menu())

    elif data == "btc_alert_on":
        if user_id not in SUBSCRIBERS:
            SUBSCRIBERS[user_id] = {"btc_alert": False, "gold_alert": False}
        SUBSCRIBERS[user_id]["btc_alert"] = True
        await query.message.reply_text("🔔 BTC Alert ON!\n\nAnda akan dapat notifikasi bila harga BTC masuk zon entry.", reply_markup=main_menu())
        ...
        await query.message.reply_text("🔔 GOLD Alert ON!\n\nAnda akan dapat notifikasi bila harga GOLD masuk zon entry.", reply_markup=main_menu())
        ...
        await query.message.reply_text("✅ Berjaya Subscribe!\n\n🔔 BTC Alert ON\n🔔 GOLD Alert ON\n\nAnda akan dapat notifikasi setiap 5 minit bila harga masuk zon.", reply_markup=main_menu())

    elif data == "unsubscribe":
        if user_id in SUBSCRIBERS:
            del SUBSCRIBERS[user_id]
        await query.message.reply_text("❌ Berjaya Unsubscribe.\n\nSemua alert telah dimatikan.", reply_markup=main_menu())
            ...
            try:
                if settings.get("btc_alert", False) and btc_active:
                    ...
                    if key not in ALERT_CACHE or ALERT_CACHE[key] == False:
                        alert_msg = (
                            "🚨 ALERT ZON ENTRY AKTIF!\n\n"
                            "₿ BTC - Zon: " + str(btc_type) + "\n"
                            "💰 Entry: $" + str(btc_entry) + "\n"
                            "🎯 Take Profit: $" + str(btc_tp) + "\n"
                            "🛑 Stop Loss: $" + str(btc_sl) + "\n\n"
                            "⚡ Harga semasa berada dalam zon!\n"
                            "Pertimbangkan untuk entry sekarang."
                        )
                ...
                if settings.get("gold_alert", False) and gold_active:
                    ...
                    if key not in ALERT_CACHE or ALERT_CACHE[key] == False:
                        alert_msg = (
                            "🚨 ALERT ZON ENTRY AKTIF!\n\n"
                            "🥇 GOLD - Zon: " + str(gold_type) + "\n"
                            "💰 Entry: $" + str(gold_entry) + "\n"
                            "🎯 Take Profit: $" + str(gold_tp) + "\n"
                            "🛑 Stop Loss: $" + str(gold_sl) + "\n\n"
                            "⚡ Harga semasa berada dalam zon!\n"
                            "Pertimbangkan untuk entry sekarang."
                        )
                        await app.bot.send_message(chat_id=user_id, text=alert_msg)
