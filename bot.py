...
def get_live_price(asset):
    if asset == "btc":
        apis = [
            ("https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=usd", lambda r: r.json().get("bitcoin", {}).get("usd")),
            ("https://api.binance.com/api/v3/ticker/price?symbol=BTCUSDT", lambda r: float(r.json().get("price", 0))),
        ]
    else:
        apis = [
...
def check_news_risk(news):
    ...
    for e in news:
        try:
            ...
            if diff <= 30: return True, f"HIGH IMPACT NEWS dalam {int(diff)} min: {e['title']}"
        except: pass
    return False, ""
...
def rsi_divergence(c, lb=10):
    ...
    for i in range(len(closes)):
        if i >= 14: rv.append(rsi_calc(closes[max(0, i-28):i+1]))
        else: rv.append(None)
