...
def save_history(asset, direction, price, score):
    ...
    except Exception as e: log.warning("history error: %s", e)
...
def binance_candles(symbol, interval="15m", limit=200):
    try:
        r = SESSION.get("https://api.binance.com/api/v3/klines", params={"symbol": symbol, "interval": interval, "limit": limit}, timeout=10)
        ...
        return out
    except Exception as e: log.warning("binance %s: %s", symbol, e); return []
...
def coingecko_ohlc(coin, days=90):
    try:
        ...
        out = []
        for x in r.json():
            try: out.append({"time": x[0], "open": float(x[1]), "high": float(x[2]), "low": float(x[3]), "close": float(x[4]), "volume": 0})
            except: pass
        return out
    except Exception as e: log.warning("coingecko %s: %s", coin, e); return []

def twelvedata_candles(symbol, interval="15min", size=200):
    if not TWELVE_KEY: return []
    try:
        r = SESSION.get("https://api.twelvedata.com/time_series", params={"symbol": symbol, "interval": interval, "outputsize": size, "apikey": TWELVE_KEY}, timeout=10)
        if r.status_code != 200 or r.json().get("status") == "error": return []
        out = []
        for x in reversed(r.json().get("values", [])):
            try: out.append({"time": x.get("datetime"), "open": float(x.get("open", 0)), "high": float(x.get("high", 0)), "low": float(x.get("low", 0)), "close": float(x.get("close", 0)), "volume": 0})
            except: pass
        return out
    except Exception as e: log.warning("twelvedata %s: %s", symbol, e); return []
...
def gold_candles():
    ...
    out = []
    for c in paxg:
        out.append({"time": c["time"], "open": c["open"]*ratio, "high": c["high"]*ratio, "low": c["low"]*ratio, "close": c["close"]*ratio, "volume": c["volume"]})
    return out, "XAUUSD"

def get_candles(asset, tf="15m", minimum=20):
    if asset == "btc":
        sources = [lambda: (binance_candles("BTCUSDT", "15m", 200), "B15"), lambda: (binance_candles("BTCUSDT", "1h", 200), "B1h"), lambda: (coingecko_ohlc("bitcoin", 90), "CG")]
    else:
        if tf in ("1h", "4h"):
            sources = [lambda: (twelvedata_candles("XAU/USD", "1h", 200), "T1h"), lambda: (binance_candles("PAXGUSDT", "1h", 200), "P1h"), lambda: gold_candles()]
        else:
            sources = [lambda: (twelvedata_candles("XAU/USD", "15min", 200), "T15"), lambda: gold_candles(), lambda: (coingecko_ohlc("pax-gold", 90), "CG")]
    for fn in sources:
        try:
            result = fn()
            candles, src = result if isinstance(result, tuple) else (result, "?")
            if candles and len(candles) >= minimum: return candles, src
        except Exception as e: log.warning("candle src: %s", e)
    return [], None

def get_news():
    try:
        r = SESSION.get("https://nfs.faireconomy.media/ff_calendar_thisweek.json", timeout=5)
        if r.status_code != 200: return []
        out = []
        for e in r.json():
...
def check_news_risk(news):
    ...
    for e in news:
        try:
            ...
            if diff <= 30: return True, "HIGH IMPACT NEWS dalam " + str(int(diff)) + " min: " + e["title"]
        except: pass
    return False, ""
...
def structure(c):
    ...
    a, b = c[-20:-10], c[-10:]
    if max(x["high"] for x in b) > max(x["high"] for x in a) and min(x["low"] for x in b) > min(x["low"] for x in a): return "BULLISH"
    if max(x["high"] for x in b) < max(x["high"] for x in a) and min(x["low"] for x in b) < min(x["low"] for x in a): return "BEARISH"
    return "NEUTRAL"
...
def candle_conf(c):
    ...
    xb = x["close"] > x["open"]
    xa = x["close"] < x["open"]
    if p["close"] < p["open"] and xb and x["open"] <= p["close"] and x["close"] >= p["open"]: return "BULLISH ENGULFING"
    if p["close"] > p["open"] and xa and x["open"] >= p["close"] and x["close"] <= p["open"]: return "BEARISH ENGULFING"
    rng = x["high"] - x["low"]
    body = abs(x["close"] - x["open"])
    if rng > 0 and body/rng >= 0.65: return "BULLISH CANDLE" if xb else "BEARISH CANDLE"
    return "NONE"

def liq_sweep(c, w=20):
    if len(c) < w+1: return "NONE", None
    x = c[-1]
    prev = c[-(w+1):-1]
...
def rsi_divergence(c, lb=10):
    if len(c) < lb+14: return "NONE"
    closes = [x["close"] for x in c]
    rv = [rsi_calc(closes[max(0,i-28):i+1]) if i >= 14 else None for i in range(len(closes))]
    rr = [x for x in rv[-lb:] if x is not None]
    if len(rr) < 2: return "NONE"
    pp = [x["close"] for x in c[-len(rr):]]
    if pp[-1] < pp[0] and rr[-1] > rr[0]: return "BULLISH DIV"
    if pp[-1] > pp[0] and rr[-1] < rr[0]: return "BEARISH DIV"
    return "NONE"
...
def trend(cx):
    if len(cx) < 20: return "NEUTRAL"
    hc = [x["close"] for x in cx]
    a = ema(hc, 20)
    b = ema(hc, 50) if len(hc) >= 50 else None
    if a and b: return "BULLISH" if a > b else "BEARISH"
    return "NEUTRAL"
...
def analyze(asset):
    ...
    if not news_risk:
        if bias == "BUY":
            full = liq=="BULLISH SWEEP" and candle in ("BULLISH ENGULFING","BULLISH CANDLE") and bos=="BULLISH BOS" and retest=="BULLISH RETEST"
            part = score>=75 and candle in ("BULLISH ENGULFING","BULLISH CANDLE") and bos=="BULLISH BOS" and retest=="BULLISH RETEST"
            if full or part: direction = "BUY"
        elif bias == "SELL":
            full = liq=="BEARISH SWEEP" and candle in ("BEARISH ENGULFING","BEARISH CANDLE") and bos=="BEARISH BOS" and retest=="BEARISH RETEST"
            part = score>=75 and candle in ("BEARISH ENGULFING","BEARISH CANDLE") and bos=="BEARISH BOS" and retest=="BEARISH RETEST"
            if full or part: direction = "SELL"
    confidence = min(95, int(55+score*0.40)) if direction in ("BUY","SELL") else min(59, int(40+score*0.10))
    safe_av = (av*scale) if av else price*0.005
    ref = bosp if bosp else lp
    zl = (ref - safe_av*0.20) if ref else (price - safe_av*0.20)
    zh = (ref + safe_av*0.20) if ref else (price + safe_av*0.20)
    ...
    reasons = []
    if e20 and e50: reasons.append("EMA20 " + ("atas" if e20>e50 else "bawah") + " EMA50")
    ...
    if direction == "WAIT":
        if bias == "BUY":
            if candle not in ("BULLISH ENGULFING","BULLISH CANDLE"): missing.append("Bullish candle")
            if bos != "BULLISH BOS": missing.append("Bullish BOS")
            if retest != "BULLISH RETEST": missing.append("Bullish retest")
        elif bias == "SELL":
            if candle not in ("BEARISH ENGULFING","BEARISH CANDLE"): missing.append("Bearish candle")
            if bos != "BEARISH BOS": missing.append("Bearish BOS")
            if retest != "BEARISH RETEST": missing.append("Bearish retest")
        else: missing.append("Directional bias")
    return {"market_open": True, "price": price, "direction": direction, "bias": bias, "confidence": confidence, "score": score, "ema20": e20, "ema50": e50, "rsi": rv, "atr": av, "adx": ax, "divergence": div, "structure": st, "h1": h1, "h4": h4, "liq": liq, "liq_price": lp, "candle": candle, "bos": bos, "bos_price": bosp, "retest": retest, "retest_price": retp, "sl": sl, "tp1": tp1, "tp2": tp2, "rr1": rr1, "rr2": rr2, "zone_low": zl, "zone_high": zh, "swing_low": slw, "swing_high": shw, "reasons": reasons, "missing": missing, "news": news, "news_risk": news_risk, "news_msg": news_msg, "source": s15, "session": get_session()}
...
def fmt(asset, d):
    ...
    price = d["price"]
    if direction == "BUY": sig = "SIGNAL: BUY"
    elif direction == "SELL": sig = "SIGNAL: SELL"
    else: sig = "SIGNAL: TUNGGU"
    lines = ["━━━━━━━━━━━━━━━━━━━━", "📊 " + name, "━━━━━━━━━━━━━━━━━━━━", sig, "💰 Harga: " + str(round(price,2)), "🎯 Bias: " + d["bias"], "📈 Score: " + str(d["score"]) + "/100", "🔒 Confidence: " + str(d["confidence"]) + "%", "🕐 Session: " + d.get("session",""), "📡 Source: " + str(d.get("source","")), ""]
    if d.get("rsi"): lines.append("RSI: " + str(round(d["rsi"],1)))
    if d.get("adx"): lines.append("ADX: " + str(round(d["adx"],1)))
    if d.get("atr"): lines.append("ATR: " + str(round(d["atr"],2)))
    ...
    if direction in ("BUY","SELL"):
        lines.append("")
        lines.append("🎯 Level:")
        if d.get("sl"): lines.append("  SL : " + str(round(d["sl"],2)))
        if d.get("tp1"): lines.append("  TP1: " + str(round(d["tp1"],2)) + " (RR " + str(d.get("rr1","")) + ")")
        if d.get("tp2"): lines.append("  TP2: " + str(round(d["tp2"],2)) + " (RR " + str(d.get("rr2","")) + ")")
        if d.get("zone_low"): lines.append("  Zone: " + str(round(d["zone_low"],2)) + " - " + str(round(d["zone_high"],2)))
    if d.get("news"):
        lines.append("")
        lines.append("📰 News USD:")
        for n in d["news"][:3]: lines.append("  • " + n.get("title",""))
    lines.append("━━━━━━━━━━━━━━━━━━━━")
    return "\n".join(lines)

async def auto_alert_loop(app):
    while True:
            ...
            try:
                for asset in ["gold","btc"]:
                    ...
                    if d and d.get("market_open"):
                        ...
                        if curr != "WAIT" and curr != prev:
                            last_signals[(chat_id,asset)] = curr
                            await app.bot.send_message(chat_id=chat_id, text=fmt(asset,d))
...
async def gold_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        log.error(e)
        await update.message.reply_text("Ralat analisis gold.")
...
async def btc_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        log.error(e)
        await update.message.reply_text("Ralat analisis btc.")

async def auto_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    auto_chats.add(update.effective_chat.id)
    await update.message.reply_text("✅ Auto Alert AKTIF! Bot akan check setiap 5 minit.")
...
async def stats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for asset in ("gold","btc"):
        ...
        if s: lines.append(name + ": BUY " + str(s["buy"]) + " | SELL " + str(s["sell"]) + " | WAIT " + str(s["wait"]))
        else: lines.append(name + ": Tiada data")
    await update.message.reply_text("\n".join(lines))

async def main():
    if not BOT_TOKEN:
        log.error("BOT_TOKEN tidak dijumpai!")
        return
    ...
    asyncio.get_event_loop().create_task(auto_alert_loop(app))
    log.info("Bot dimulakan...")
